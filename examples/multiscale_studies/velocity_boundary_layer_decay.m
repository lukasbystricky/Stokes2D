...
input_params = default_input_params('decay_study', 1);
...
n_layers = 10;
H = input_params.box_size(2)/2;
A = H*input_params.box_size(1);
c = 0.2;% concentration
h = H / n_layers; % spacing between circle centers

radii = sqrt(A *c /(pi*n_layers))*ones(n_layers, 1);
centers = linspace(h/2, H-h/2, n_layers)' - Ly/2;

...
//#pragma omp parallel for
    for(int j = 0;j<Ntar;j++) {
        ...
        Complex nzpan32[32], p32[33], tn32[32];
                ...
                if (pk > -1) { // Only panels not equal to -1 will be considered
                    ...
                    if (check_sq) {
                        ...
                        if (!accurate) {
                            ...
                            double sum16;
                            double Ic16real = 0;
                            double r1, r2, dens1, dens2, rsq;
                            
                            for (int k = 0; k<16; k++) {
                                // pressure is -imag(Ic[q/n])/(2*pi)
                                Ic16 += tf[k] * tW[k] * tzp[k]/(tn[k]*-(z - tz[k]));
                                ...
                                Ic16real += (r1*dens1 + r2*dens2)/rsq;
                            }
                            ...
                            if (accurate) {
                                ...
                                for (int k=0; k<32; k++) {
                                    // pressure is -imag(Ic[q/n])/(2*pi)
                                    Ic32 += tf32[k] * tW32[k] * tzp32[k]/(tn32[k]*-(z - tz32[k]));
                                ...
                                for (int k = 0; k<32; k++) {                                    
                                    f = tf32[k] / tn32[k];
                                    Ic_helsing += p32[k]*f; //minus sign because we're actually interested in 1/(z-t)
                                }
                                
                                double modif = -imag(Ic_helsing)/(2*pi) - sum16;
                                out_p[j] += modif;
                            }

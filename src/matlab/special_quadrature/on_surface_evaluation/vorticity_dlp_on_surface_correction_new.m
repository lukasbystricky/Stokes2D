function omegac = vorticity_dlp_on_surface_correction_new(omega, solution_local, lim_dir)
%VORTICITY_DLP_ON_SURFACE_CORRECTION corrects the double-layer 
%vorticity for on-surface evaluation. Adds on the jump as the target
%approaches the boundary from different directions.
%
%inputs:
% -omega: uncorrected double-layer vorticity evaluated at quadrature points
% on surface
...
for i = 1:size(domain_local.wall_indices,1)
    ...
    for j = thiswall
        ...
        Ih = sum((qtmp./r.^2).*wtmp.*zptmp);
        
        omegac(j) = omegac(j) - -real(Ih)/pi;
    ...
    if periodic_rep
        for j = [1 panels_per_wall]
            ...
            for k = 1:length(target_ind)
                ...
                omegac(tar_ind_tmp) = omegac(tar_ind_tmp) - -real(Ih)/pi;
    ...
    Ih = on_surface_evaluation(2,qtmp,ztmp,zptmp,wtmp,panel_breaks_z,...
        lim_dir,nbr_neighbor_pts,periodic_rep,reference_cell);

    % add on correction
    omegac(thiswall) = omegac(thiswall) + -real(Ih)/pi; 

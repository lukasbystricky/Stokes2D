...
if solution.problem.periodic
%     omega_slp_ds = evaluate_single_layer_vorticity_direct(X(:),Y(:),...
%         domain.z,domain.zp,solution.q(:,1)+1i*solution.q(:,2),domain.quad_weights);
% 
%     [omega_slp_corrected_ds,~] = mex_SQ_slp_vorticity(X(:)+1i*(Y(:)+1e-60), ...
...
%         omega_slp_ds, domain.mean_panel_length, domain.extra.gridSolidmat, ...
...
%         omega_dlp_ds = evaluate_double_layer_vorticity_direct(X(:),Y(:),...
%             domain.z,domain.zp,solution.q(:,1)+1i*solution.q(:,2),domain.quad_weights);
%     
%         % correct using special quadrature
%         [omega_dlp_corrected_ds,~] = mex_SQ_dlp_vorticity(X(:)+1i*(Y(:)+1e-60), domain.z, domain.zp,...
%             domain.quad_weights, domain.panel_breaks, domain.wazp, domain.z32,...
%             domain.zp32, domain.quad_weights32, domain.wazp32,...
%             solution.q(:,1)+1i*solution.q(:,2),real(omega_dlp_ds)+1i*(imag(omega_dlp_ds)+1e-60),domain.mean_panel_length,...

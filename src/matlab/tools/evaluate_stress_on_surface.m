...
if solution.problem.periodic
    ...
    %sigma1_slp_corrected = stress_slp_on_surface_correction2(sigma1_slp, b1, solution, solution_local, type);
    %sigma2_slp_corrected = stress_slp_on_surface_correction2(sigma2_slp, b2, solution, solution_local, type);
    
    %norm(sigma1_slp_corrected_old-sigma1_slp_corrected)
    %norm(sigma2_slp_corrected_old-sigma2_slp_corrected)
    
%     [sigma1_slp_corrected,nmod1] = mex_SQ_slp_stress(xtar(:)+1i*(ytar(:)+1e-60),...
...
%     
%     [sigma2_slp_corrected,nmod2] = mex_SQ_slp_stress(xtar(:)+1i*(ytar(:)+1e-60),...
%         xsrc(:)+1i*(ysrc(:)+1e-60), domain.zp, domain.quad_weights, domain.panel_breaks,...
%         domain.wazp, domain.z32, domain.zp32, domain.quad_weights32, domain.wazp32, ...
...
%     norm(sigma1_slp_corrected-sigma1_slp_corrected_mex)
%     norm(sigma2_slp_corrected-sigma2_slp_corrected_mex)
%    disp([nmod1 nmod2]);
%     
...
%             domain.reference_cell,true);
    ...
    zsrc = solution.problem.domain.z;
...
%         domain.reference_cell,false);
    ...
    sigma1 = sigma1_dlp_corrected + sigma1_S + sigma1_R;
    sigma2 = sigma2_dlp_corrected + sigma2_S + sigma2_R;
end

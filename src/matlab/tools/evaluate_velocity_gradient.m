...
[ug1_slp_corrected,~] = mex_SQ_slp_velocity_grad(X(:)+1e-60+1i*(Y(:)+1e-60), domain.z, domain.zp,...
...
% correct using special quadrature
[ug2_slp_corrected,~] = mex_SQ_slp_velocity_grad(X(:)+1e-60+1i*(Y(:)+1e-60), domain.z, domain.zp,...
    domain.quad_weights, domain.panel_breaks, domain.wazp, domain.z32,...
    domain.zp32, domain.quad_weights32, domain.wazp32, ...
...
if isinf(solution.problem.eta)
    ...
    % correct using special quadrature
    [ug1_dlp_corrected,~] = mex_SQ_dlp_velocity_grad(X(:)+1e-60+1i*(Y(:)+1e-60), domain.z, domain.zp,...
        domain.quad_weights, domain.panel_breaks, domain.wazp, domain.z32,...
        domain.zp32, domain.quad_weights32, domain.wazp32, ...
        solution.q(:,1)+1i*solution.q(:,2), ones(ntar,1) + 1e-60*1i*ones(ntar,1),...
    ...
    % correct using special quadrature
    [ug2_dlp_corrected,~] = mex_SQ_dlp_velocity_grad(X(:)+1e-60+1i*(Y(:)+1e-60), domain.z, domain.zp,...
        domain.quad_weights, domain.panel_breaks, domain.wazp, domain.z32,...
        domain.zp32, domain.quad_weights32, domain.wazp32, ...
        solution.q(:,1)+1i*solution.q(:,2), 1e-60*ones(ntar,1) + 1i*ones(ntar,1),...

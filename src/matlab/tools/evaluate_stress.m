...
if solution.problem.periodic
    [sigma1x_slp, sigma2x_slp] = StokesSLP_stress_ewald_2p(xsrc, ysrc, X(:), Y(:),...
        solution.q(:,1).*weights, solution.q(:,2).*weights, ones(ntar,1),...
        zeros(ntar,1), Lx, Ly, 'verbose', 0);

    [sigma1y_slp, sigma2y_slp] = StokesSLP_stress_ewald_2p(xsrc, ysrc, X(:), Y(:),...
        solution.q(:,1).*weights, solution.q(:,2).*weights, zeros(ntar,1),...
        ones(ntar,1), Lx, Ly, 'verbose', 0);

    sigma1_slp = sigma1x_slp + 1i*sigma2x_slp;
    sigma2_slp = sigma1y_slp + 1i*sigma2y_slp;

    if isinf(solution.problem.eta)
        sigma1 = sigma1_slp;
        sigma2 = sigma2_slp;
    else
        [sigma1x_dlp, sigma2x_dlp] = StokesDLP_stress_ewald_2p(xsrc, ysrc, X(:), Y(:),...
            n1, n2,solution.q(:,1).*weights, solution.q(:,2).*weights, ones(ntar,1),...
            zeros(ntar,1), Lx, Ly, 'verbose', 0);

        [sigma1y_dlp, sigma2y_dlp] = StokesDLP_stress_ewald_2p(xsrc, ysrc, X(:), Y(:),...
            n1, n2, solution.q(:,1).*weights, solution.q(:,2).*weights, zeros(ntar,1),...
            ones(ntar,1), Lx, Ly, 'verbose', 0);

        sigma1_dlp = sigma1x_dlp + 1i*sigma2x_dlp;
        sigma2_dlp = sigma1y_dlp + 1i*sigma2y_dlp;
    ...
    % correct using special quadrature
    [sigma1_slp_corrected,~] = mex_SQ_dlp(Xtar_sq(:) + 1i*Ytar_sq(:),...
    ...
    [sigma2_slp_corrected,~] = mex_SQ_dlp(Xtar_sq(:) + 1i*Ytar_sq(:),...
        domain.z, domain.zp, domain.quad_weights, domain.panel_breaks,...
        domain.wazp, domain.z32, domain.zp32, domain.quad_weights32, domain.wazp32, ...
        solution.q(:,1)+1i*solution.q(:,2),...
